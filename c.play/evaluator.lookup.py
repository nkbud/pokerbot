...
class LookupTable(object):
    ...
    Straight Flush   10
    Four of a Kind   156      [(13 choose 1) * (12 choose 1)]
    Full Houses      156      [13 * (2 choose 1)]
    ...
    * 7-5-4-3-2 unsuited (worst hand possible)  => 7462
    ...
    def rank_to_prime_product(self) -> {}:
        rank2prime = {}
        for k,v in self.flush_lookup.items():
            rank2prime[v] = k
        for k,v in self.unsuited_lookup.items():
            rank2prime[v] = k
        return rank2prime
    ...
    def __init__(self):
        ...
        # opposite direction
        self.rank2prime = self.rank_to_prime_product()
    ...
    @staticmethod
    def find_rank_class(rank):
        for rank_class in range(len(LookupTable.RANK_CLASS_TO_MAX)):
            if rank <= LookupTable.RANK_CLASS_TO_MAX[rank_class]:
                return rank_class
    ...
    def write_table_to_disk(self,table,filepath):
        ...
        with open(filepath,"w") as f:
            ...
            for prime_prod,rank in sorted(table.items(),key=lambda x:x[1]):
                kinds = EvaluationCard.cards_from_prime_product(prime_prod)
                rank_class = LookupTable.find_rank_class(rank)
                frequency = LookupTable.RANK_CLASS_TO_FREQUENCY[rank_class]
                cumulative_frequency += frequency
                f.write(f"{prime_prod},{kinds},{rank},{frequency},{cumulative_frequency}\n")
